...
bool lex_source(lvl2_line_t** lines, int line_num, uint8_t** opcodes, int* opcode_num) {
    ...
    //Replace the label references with their addresses
    for (int i = 0; i < label_reference_num; i++) {
        uint64_t address = 0;
        for (int j = 0; j < label_definition_num; j++) {
            if (strcmp(label_definitions[j].name, label_references[i].name) == 0) {
        ...
        if (address) {
            //Replace the opcode with the address
            uint8_t* opcode = &(*opcodes)[label_references[i].local_address];
            memcpy(opcode, &address, sizeof(uint64_t));
        } else {
            printf("Reference to undefined label %s!\n", label_references[i].name);
            return true;
        }
